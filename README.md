...
> **下一代 PHP Web 应用防火墙 · 14 层编码归一化 · 11 解析器语义引擎 · 贝叶斯机器学习 · 双证据融合 · 双重密码加密**
> **署名：暗夜铭少**

---

## 🎉 v5.3.0 极限突破：检测率 100%，误报率 0%
...
**核心优化**：
- 🔧 语义引擎路由逻辑重构：路径遍历/命令注入/PHP/HTML解析器路由全部修复
- ⚡ 快速短路智能扩展：新增5类攻击特征例外，攻击不遗漏，正常请求零开销
- 🛡️ 缓存投毒防护修复：localhost误判问题彻底解决
- 📊 熵值分析优化：Base64/URL编码等正常内容智能识别，误报清零
- 🎯 SSRF/SSTI评分增强：高危特征权重全面提升
...
| 🧠 **语义分析引擎** | 11 个 AST/DOM 级别语义解析器 + 6 个上下文分析器，内容类型路由 + 双证据增强 |
| 🤖 **贝叶斯机器学习** | 纯 PHP 零依赖朴素贝叶斯分类器，自动学习攻击/正常模式，越用越准 |
| 🔗 **双证据融合架构** | 规则引擎 + 语义引擎 + ML 概率，三路评分互相印证，杜绝绕过 |
...
├─────────────────────────────────────────────────────────────┤
│  Layer 3  语义分析 · 11 解析器(AST/DOM) · 内容路由 · 双证据融合         │
├─────────────────────────────────────────────────────────────┤
...
### 11 个语义解析器（全部 AST/DOM 级别）

**8 级深度体系定义：**

| 级别 | 名称 | 说明 |
|------|------|------|
| Level 1 | 关键词匹配 | 正则/字符串匹配 |
| Level 2 | 模式匹配 | 多特征组合启发式 |
| Level 3 | 结构化解析 | 识别基本结构单元 |
| Level 4 | **AST/DOM 构建** | 完整语法树解析 |
| Level 5 | **语义分析** | 类型推断、数据流、污点标记 |
| Level 6 | **控制流/数据流** | CFG 构建、污点传播、执行链追踪 |
| Level 7 | **语义等价分析** | 攻击变异识别、意图推理、沙箱逃逸链 |
| Level 8 | 符号执行 | 约束求解、路径可达性 |

| 分类 | 解析器 | 深度级别 | 核心能力 |
|------|--------|---------|---------|
| **注入类** | SQL 语义解析器 | **Level 7** 语义等价 | AST + 污点传播 + 语义等价 + 攻击意图推理 |
| **代码类** | PHP 代码语义解析器 | **Level 6** 数据流 | Tokenizer + 变量跟踪 + 污点传播 + 控制流分析 |
| **XSS 类** | HTML/XSS 语义解析器 | **Level 6** 执行链 | DOM + 事件流分析 + 沙箱逃逸 + DOM XSS 检测 |
| **代码类** | 命令注入解析器 | **Level 5** 语义分析 | Shell AST + 管道数据流 + 意图推理 + 逃逸绕过检测 |
| **代码类** | SSTI 模板注入解析器 | Level 4 模板 AST | Tokenizer + 递归下降 + 过滤器链 + 危险函数 |
| **包含类** | SSRF 语义解析器 | Level 4 URL AST | Tokenizer + 完整 URL 结构 + 内网/云元数据检测 |
| **包含类** | 路径遍历解析器 | Level 4 路径 AST | Tokenizer + 路径规范化 + 编码绕过检测 |
| **代码类** | 反序列化解析器 | Level 4 序列化 AST | Tokenizer + 递归下降 + 危险类 + POP 链特征 |
| **代码类** | XXE 实体解析器 | Level 4 XML DOM | DOMDocument + DOMXPath + 实体扩张 + XInclude |
| **XSS 类** | CRLF 注入解析器 | Level 4 HTTP 头 AST | Tokenizer + 头结构 + 响应拆分 + 缓存投毒 |
| **业务类** | 表达式注入解析器 | Level 4 表达式 AST | Tokenizer + 递归下降 + 危险函数 + 动态变量 |
...
- **贝叶斯机器学习**：纯 PHP 朴素贝叶斯分类器，自动学习攻击/正常模式，冷启动保护
- **沙箱联动**：高置信度恶意样本自动投喂
- **人工审核**：中风险样本标记待审核，避免误报污染
- **增量学习**：支持在线持续学习，模型 JSON 持久化
...
| 项目 | 最低要求 | 说明 |
|------|---------|------|
| PHP 版本 | 7.0+ | 推荐 7.4+，PHP 8 性能更好 |
| 内存 | 128MB | WAF 自身占用 ~2-6MB |
| CPU | 1核 | 单请求 ~1-10ms |
| 磁盘 | 10MB | 日志和缓存占用很小 |
| 扩展 | 无强制要求 | OPcache 强烈建议开 |

> 最低配置请使用 `WAF_PERFORMANCE_MODE=high` 高性能模式

### 推荐配置（大部分站点）

| 项目 | 推荐配置 | 说明 |
|------|---------|------|
| PHP 版本 | 7.4+ | PHP 8.1+ 性能最优 |
| 内存 | 256MB+ | 给 PHP-FPM 留足空间 |
| CPU | 2核 | 抗住正常访问量 |
| 磁盘 | 50MB+ | 日志量大可以调大 |
| 扩展 | OPcache + APCu | 性能提升 2-3 倍 |

> 推荐配置使用默认 `WAF_PERFORMANCE_MODE=balanced` 平衡模式

### 全功能配置（strict 模式）

开启所有功能（11个解析器 + 记忆池 + 攻击链 + 跨请求 + 贝叶斯 + 主动防御 + 蜜罐）需要：
...
| CPU | **4核+** |
| 扩展 | **OPcache + APCu 必开** |
| PHP-FPM | max_children >= 16 |
| 单请求耗时 | ~5-15ms |

| 开启的功能 | 额外内存占用 | 额外CPU耗时 |
|-----------|:-----------:|:----------:|
| 11个语义解析器 | ~1MB | ~2-5ms |
| 语义记忆池(L9) | ~0.5MB | ~0.5ms |
| 攻击链分析 | ~0.3MB | ~0.3ms |
| 参数位置分析(L11) | ~0.2MB | ~0.2ms |
| 跨请求上下文(L12) | ~1MB | ~1ms |
| 贝叶斯机器学习 | ~0.5MB | ~0.5ms |
| 主动防御/蜜罐 | ~0.3MB | ~0.2ms |
| **总计（全开）** | **~4-6MB** | **~5-8ms** |

> 全功能建议服务器至少 2核2G，4核4G 体验最佳
...
| 单请求耗时 | ~1-3ms | ~2-6ms | ~5-15ms |
| 内存占用 | ~2-3MB | ~4-6MB | ~6-8MB |
...
**high 高性能模式**
- 保留核心防护能力：11个语义解析器 + 14层编码归一化 + 规则引擎 + 智能评分 + 7层误报控制
- 关闭跨请求重型分析：记忆池、攻击链、跨请求上下文、贝叶斯ML、主动防御
- 适合：1核1G虚拟主机、访问量大的站点、追求极致性能

**balanced 平衡模式（默认）**
- 功能全开，性能与安全兼顾
- 适合：2核2G及以上服务器、大部分网站、推荐配置

**strict 严格模式**
- 所有功能拉满，最高安全性
- 适合：4核4G及以上服务器、高安全需求站点、政企/金融类网站

#### 怎么切换模式？

```env
# 在 .env 文件里加一行即可
WAF_PERFORMANCE_MODE=high    # 高性能
# WAF_PERFORMANCE_MODE=balanced  # 平衡（默认）
# WAF_PERFORMANCE_MODE=strict    # 严格
```
